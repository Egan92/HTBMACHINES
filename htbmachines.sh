...
function helpPanel(){
  ...
  echo -e "\t${purpleColour}y)${endColour}${grayColour} Obtener link del Write Up de Youtube${endColour}"
...
function updateFiles(){
  if [ ! -f bundle.js ]; then
    ...
    tput civis
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} Comprobando si hay actualizacines pendientes...${endColour}"
    ...
    if [ "$md5_temp_value" == "$md5_original_value" ]; then
      ...
      echo -e "\n${yellowColour}[+]${endColour}${grayColour} Se han encontrado actualizacines disponibles${endColour}"
      sleep 1
...
function getYoutubelink(){
  ...
  if [ $youtubelink ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} El Write Up para esta máquina está en el siguiente enlace:${endColour}${blueColour} $youtubelink${endColour}\n"
  else
...
function getOSDifficultyMachines(){
  ...
  if [ "$check_results" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} Listando máquinas de dificultad ${endColour}${blueColour} $difficulty${endColour}${grayColour} y con sistema operativo${endColour}${purpleColour} $os${endColour}${grayColour}:${endColour}\n"
    cat bundle.js | grep "so: \"$os\"" -C 4 | grep "dificultad: \"$difficulty\"" -B 5 | grep "name:" | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column
  else
    echo -e "\n${redColour}[!] El sistema operativo o la dificultad indicados no existen${endColour}\n"
  fi
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} Representando las máquinas que poseen un la skill${endColour}${blueColour} $skill${endColour}${grayColour}:${endColour}\n"
